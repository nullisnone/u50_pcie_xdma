...
#define SEND_BUF_SIZE (ROWS * COLS * 2) // 131072 bytes (2 matrices, uint8)
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    // Fill buffer with Matrix 1 and Matrix 2 data
    int buf_idx = 0;
    // Matrix 1 write firstly
    for (int r = 0; r < ROWS; ++r) {
        for (int c = 0; c < COLS; ++c) {
            int val = (r < matrices[1].rows) ? matrices[1].data[r][c] : 0;
            send_buf[buf_idx++] = (uint8_t)val;
        }
    }
    // Matrix 0 write secondly
    for (int r = 0; r < ROWS; ++r) {
        for (int c = 0; c < COLS; ++c) {
    ...
    // Direct Memory Write
    // HLS Kernel expects Matrix A at 0x300000 and Matrix B at 0x304000 (DWORD addressing)
    // The range 0x300000 to 0x308000 covers 32K DWORDs = 128KB.
    const uint64_t host_map_bar2_addr = 0x000000;
    int ret_write = ndpp_direct_memory_write(ndpp_dev, host_map_bar2_addr, send_buf, SEND_BUF_SIZE);
    ...
    printf("send_buf ndpp_direct_memory_write successful. Sent %d:%d:%d bytes.\n", SEND_BUF_SIZE, (int)sizeof(send_buf), ret_write);
    free(send_buf);
    g_send_buf = NULL;
